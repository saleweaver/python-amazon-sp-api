...
class FbaInboundEligibility(Client):
    ...
    def get_item_eligibility_preview_extended(self, **kwargs) -> ApiResponse:
        ...
        **Usage Plan:**
        ...
        filter = self._request(kwargs.pop('path'), params=kwargs)

        if filter.payload.get("ineligibilityReasonList") and filter.payload.get("isEligibleForProgram") is False:
            ineligibility_list = filter.payload.get("ineligibilityReasonList")
            errors = []
