import pytest
from sp_api.api.merchant_fulfillment.merchant_fulfillment import \
    MerchantFulfillment
        ...
        res = MerchantFulfillment().get_eligible_shipment_services_old(
            {
                ...
                "ItemList": [{"OrderItemId": "52986411826454", "Quantity": 1}],
                "ShipFromAddress": {
                    "Name": "John Doe",
...
def test_get_shipment_400():
    with pytest.raises(SellingApiForbiddenException) as info:
        try:
            res = MerchantFulfillment().get_shipment(
                "aabbccdd-1beb-4cda-8bf4-7366cfddbec1"
            )
...
def test_cancel_shipment():
    with pytest.raises(SellingApiForbiddenException) as info:
        res = MerchantFulfillment().cancel_shipment(
...
def test_cancel_shipment_old():
    with pytest.raises(SellingApiForbiddenException) as info:
        res = MerchantFulfillment().cancel_shipment_old(
            "be7a0a53-00c3-4f6f-a63a-639f76ee9253"
        )
        assert res.errors is None
        assert res.payload.get("ShipmentId") == "be7a0a53-00c3-4f6f-a63a-639f76ee9253"
