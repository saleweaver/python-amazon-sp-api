...
from sp_api.base import (Marketplaces, ProcessingStatus, Schedules,
...
def test_get_report_document_n_decrypt():
    res = Reports().get_report_document(
        "0356cf79-b8b0-4226-b4b9-0ee058ea5760", decrypt=False
    )
...
def test_get_reports_2():
    report_types = ["FEE_DISCOUNTS_REPORT", "GET_AFN_INVENTORY_DATA"]
    processing_status = [ProcessingStatus.IN_QUEUE, ProcessingStatus.IN_PROGRESS]
    res = Reports().get_reports(
        reportTypes=report_types, processingStatuses=processing_status
    )
    assert res.errors is None
...
def test_get_reports_3():
    report_types = [ReportType.FEE_DISCOUNTS_REPORT, ReportType.GET_AFN_INVENTORY_DATA]
    processing_status = [ProcessingStatus.IN_QUEUE, ProcessingStatus.IN_PROGRESS]
    res = Reports().get_reports(
        reportTypes=report_types, processingStatuses=processing_status
    )
