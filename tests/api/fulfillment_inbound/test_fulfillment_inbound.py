...
def test_item_guidance():
    res = FulfillmentInbound().item_guidance(
        SellerSKUList=",".join(["sku1", "sku2"]), MarketplaceId="MarketplaceId"
    )
            ...
            "InboundShipmentPlanRequestItems": [
                {
                    ...
                    "QuantityInCase": 1,
                    "PrepDetailsList": [
                        {"PrepInstruction": "Polybagging", "PrepOwner": "AMAZON"}
                    ],
                }
            ],
        }
...
def test_create_inbound_shipment():
    res = FulfillmentInbound().create_shipment(
        "123",
        {
...
def test_update_shipment():
    res = FulfillmentInbound().update_shipment(
        "123",
        {
...
def test_get_prep_orders():
    res = FulfillmentInbound().prep_instruction(
        {"ShipToCountryCode": "US", "ASINList": ["ASIN1"]}
    )
    assert res.errors is None
...
def test_get_shipments():
    res = FulfillmentInbound().get_shipments(
        QueryType="SHIPMENT", MarketplaceId="ATVPDKIKX0DER"
    )
    assert res.errors is None


def test_get_shipment_items():
    res = FulfillmentInbound().shipment_items_by_shipment(
        "FBA15DJ9SVVD", MarketplaceId="ATVPDKIKX0DER"
    )
    assert res.errors is None


def test_get_items():
    res = FulfillmentInbound().shipment_items(
        QueryType="SHIPMENT", MarketplaceId="ATVPDKIKX0DER", NextToken="NextToken"
    )
    assert res.errors is None
